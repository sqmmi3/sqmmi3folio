...
const aclProgress = computed(() => {
  ...
  const diffTime = Math.abs(today.getTime() - REHAB_START_DATE.getTime() - 1)
...
onMounted(() => {
  setInterval(() => {
    currentFrequency.value = Number.parseFloat((440 + (Math.random() * 0.5 - 0.25)).toFixed(2))
  }, 3000)
})
</script>
...
<template>
  <div class="min-h-screen pt-32 pb-20 px-6 max-w-6xl mx-auto">
    <section class="mb-20 animate-in" style="animation-delay: 0.1s">
      ...
      <h1
        class="text-5xl md:text-7xl font-black text-white mb-8 hover-glitch"
        data-text="THE CREATOR"
      >
        THE CREATOR
      </h1>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
        <div class="lg:col-span-2 space-y-8">
          <p class="text-gray-400 text-lg md:text-xl leading-relaxed">
            ...
            <span class="text-white">logical systems</span> and
            <span class="text-white"> creative chaos</span>. Whether I'm debugging a kernel driver,
            soldering a custom IoT node, or layering vocal harmonies in a glitchpop track, my goal
            is to build things that feel alive.
          </p>

          <div
            class="p-4 bg-red-500/10 border-l-4 border-red-500 rounded-r-xl font-mono text-sm backdrop-blur-sm"
          >
            <p class="text-red-400 font-bold mb-1">! SYSTEM_MAINTENANCE_REQUIRED</p>
            <p class="text-gray-400 italic">
              "Undergoing ACL recovery. Currently prioritizing health-sync. Responses may be
              delayed, but the grind never stops."
            </p>
          </div>
        </div>

        <div
          class="inline-flex items-center gap-4 p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-xl backdrop-blur-sm animate-in"
        ...
        >
          ...
          <div>
            <p class="text-[10px] text-emerald-500/60 font-mono uppercase tracking-widest">
              Education Level
            </p>
            <p class="text-white font-bold tracking-wide leading-tight pb-1">
              B.Sc. Computer Science (Year 3)
            </p>
            <p class="text-gray-500 text-[10px] font-mono hover-glitch" data-text="@ UCLL">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
        ...
        >
          ...
          <ul class="text-gray-500 font-mono text-xs space-y-4">
            <li>
              <span class="text-emerald-500">>></span> Role:
              <span class="text-white">Full-Stack Polymath</span>
            </li>
            <li>
              <span class="text-emerald-500">>></span> Languages:
              <span class="text-white">EN / NL / NP / FR</span>
            </li>
            <li>
          ...
          <div class="mt-8 pt-12 border-t border-white/5">
            ...
            >
              <div
                class="absolute inset-0 bg-emerald-500/10 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out"
              ></div>
              <div class="relative flex items-center justify-between">
                <div class="flex flex-col items-start">
                  <span
                    class="text-[9px] text-emerald-500 font-mono uppercase tracking-[0.3em] mb-1"
                    >Establish_Connection</span
                  >
                  <span
                ...
                <div
                  class="text-emerald-500 opacity-40 group-hover:opacity-100 transition-all duration-300 group-hover-scale-110"
                >
                  <svg
                    v-if="!isCopied"
                    xmlns="http://www.w3.org/2000/svg"
            ...
            <li v-for="stat in systemStats" :key="stat.label" class="flex flex-col gap-2">
              <div class="flex justify-between items-end font-mono">
                <div class="flex flex-col">
                  <span class="text-gray-400 text-[10px] uppercase tracking-tighter"
                    ><span class="text-emerald-500">>></span> {{ stat.label }}</span
                  >
                  <span v-if="stat.subText" class="text-[9px] text-gray-600 mt-0.5 italic">{{
                    stat.subText
                  }}</span>
                </div>
                <span :class="stat.textColor" class="text-[10px] font-bold">{{
                  stat.display
                }}</span>
              </div>
    ...
    <h2
      class="text-emerald-500 font-mono tracking-[0.3em] mb-8 uppercase text-sm animate-in"
      style="animation-delay: 0.5s"
    >
      // Equipment & Arsenal
    </h2>
    <div
      class="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in backdrop-blur-sm"
      style="animation-delay: 0.6s"
    >
      <div
        class="md:col-span-2 group relative bg-white/5 border border-white/10 p-8 rounded-2xl overflow-hidden hover:border-emerald-500/50 transition-all"
      >
        <div
          class="absolute top-0 right-0 p-4 mr-4 opacity-10 text-6xl font-black text-white italic"
        >
          TECH
        </div>
      ...
      <div
        class="group bg-white/5 border border-white/10 p-8 rounded-2xl hover:border-emerald-500/50 transition-all"
      >
      ...
      <div
        class="md:col-span-2 group bg-white/5 border border-white/10 p-8 rounded-2xl hover:border-emerald-500/50 transition-all flex flex-col justify-center"
      >
        <h3 class="text-xl font-bold text-white mb-2">Hardware Lab</h3>
        <p class="text-gray-400 text-sm mb-4 italic leading-relaxed">
          Custom IoT Nodes, ESP32, Sensors, and Embedded Systems.
        </p>
        <p
          class="text-emerald-500 text-sm font-bold tracking-tight px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-md w-fit"
        >
    ...
    <section
      class="mt-20 p-8 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl border-dashed animate-in backdrop-blur-sm"
